...
// SetupRoutes 设置路由
func SetupRoutes(uploadPath string) *gin.Engine {

	// 由于要使用中间件,所以创建方式使用New()而不是Default()
	r := gin.New()
	...
	fileuploadController := &controllers.FileUploadController{UploadPath: uploadPath}
	...
	api := r.Group("/api/v1")
	{
		...
		authenticated.Use(middleware.AuthMiddleware())
		{
			...
			// 图片上传
			uploads := authenticated.Group("/upload")
			{
				uploads.POST("", fileuploadController.UploadFile)
			}
		}
